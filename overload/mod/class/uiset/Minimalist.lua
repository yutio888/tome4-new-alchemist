...
function _M:initialize_resources()
    ...
    for res, res_def in ipairs(ActorResource.resources_def) do
        ...
        if extra_shader_params then
            -- update fenv for custom functions
...
function _M:init()
    ...
    self.mhandle_pos = {
        ...
        gamelog = { x = function(self)
            return self.logdisplay.w - move_handle[6]
        end, y = function(self)
            return self.logdisplay.h - move_handle[6]
        end, name = _t "Game Log" },
        chatlog = { x = function(self)
            return profile.chat.w - move_handle[6]
        end, y = function(self)
            return profile.chat.h - move_handle[6]
        end, name = _t "Online Chat Log" },
        hotkeys = { x = function(self)
            return self.places.hotkeys.w - move_handle[6]
        end, y = function(self)
            return self.places.hotkeys.h - move_handle[6]
        end, name = _t "Hotkeys" },
...
function _M:getMainMenuItems()
    return {
        { _t "Reset interface positions", function()
            Dialog:yesnoPopup(_t "Reset UI", _t "Reset all the interface?", function(ret)
                if ret then
                    self:resetPlaces()
                    self:saveSettings()
                end
...
function _M:resetPlaces()
    ...
    local th = 52
    if config.settings.tome.hotkey_icons then
        th = (8 + config.settings.tome.hotkey_icons_size) * config.settings.tome.hotkey_icons_rows
    end
    local hup = h - th
...
function _M:saveSettings()
    ...
    for _, w in ipairs { "player", "resources", "party", "buffs", "minimap", "gamelog", "chatlog", "hotkeys", "mainicons" } do
        ...
        if self.places[w] then
            for k, v in pairs(self.places[w]) do
...
function _M:activate()
    ...
    self.logdisplay.resizeToLines = function()
    end
    ...
    profile.chat.resizeToLines = function()
        profile.chat:resize(0 + (game.w) / 2, self.map_h_stop - font_h * config.settings.tome.log_lines - 16, (game.w) / 2, font_h * config.settings.tome.log_lines)
    end
    ...
    game.logChat = function(style, ...)
        if true or not config.settings.tome.chat_log then
            return
        end
        if type(style) == "number" then
        ...
        if game.uiset.show_userchat then
            game.uiset.logdisplay.changed = old
        end
    end
    game.logRollback = function(line, ...)
        return self.logdisplay:rollback(line, ...)
    end
    game.logNewest = function()
        return self.logdisplay:getNewestLine()
    end
...
function _M:resizeIconsHotkeysToolbar()
    local h = 52
    if config.settings.tome.hotkey_icons then
        h = (8 + config.settings.tome.hotkey_icons_size) * config.settings.tome.hotkey_icons_rows
    end
...
function _M:uiMoveResize(what, button, mx, my, xrel, yrel, bx, by, event, mode, on_change, add_text)
    ...
    elseif event == "motion" and button == "left" then
        ...
        game.mouse:startDrag(mx, my, s, { kind = "ui:move", id = what, dx = bx * self.places[what].scale, dy = by * self.places[what].scale },
                ...
                function(drag, _, x, y)
                    if self.places[drag.payload.id] then
                        self.places[drag.payload.id].x = x - drag.payload.dx
                        self.places[drag.payload.id].y = y - drag.payload.dy
                        self:boundPlaces()
                        if on_change then
                            on_change("move")
                        end
...
function _M:showResourceTooltip(x, y, w, h, id, desc, is_first)
    if not game.mouse:updateZone(id, x, y, w, h, nil, self.places.resources.scale) then
        game.mouse:registerZone(x, y, w, h, function(button, mx, my, xrel, yrel, bx, by, event)
            if is_first then
                if event == "out" then
                    self.mhandle.resources = nil
                    return
                else
                    self.mhandle.resources = true
                end
                ...
                if not self.locked and bx >= self.mhandle_pos.resources.x and bx <= self.mhandle_pos.resources.x + move_handle[6] and by >= self.mhandle_pos.resources.y and by <= self.mhandle_pos.resources.y + move_handle[7] then
                    if event == "button" and button == "right" then
                        ...
                        if player:knowTalent(player.T_FEEDBACK_POOL) then
                            list[#list + 1] = { name = _t "Feedback", id = "feedback" }
                        end
                        if player.is_fortress and player:hasQuest("shertul-fortress") then
                            list[#list + 1] = { name = _t "Fortress Energy", id = "fortress" }
                        end
...
function _M:resourceOrientStep(orient, bx, by, scale, x, y, w, h)
    if orient == "down" or orient == "up" then
        x = x + w
        if (x + w) * scale >= game.w - bx then
            x = 0
            y = y + h
        end
    elseif orient == "right" or orient == "left" then
        y = y + h
        if (y + h) * scale >= self.map_h_stop - by then
            y = 0
            x = x + w
        end
    end
...
function _M:displayResources(scale, bx, by, a)
    ...
    if player then
        ...
        if player.air < player.max_air then
            ...
            if air_sha.shad then
                air_sha:setUniform("a", a)
                air_sha.shad:use(true)
            end
            ...
            if player.air >= player.max_air * 0.5 then
                front = fshat_air
            end
        ...
        elseif game.mouse:getZone("res:air") then
            game.mouse:unregisterZone("res:air")
        end
        ...
        if life_sha.shad then
            life_sha:setUniform("a", a)
            life_sha:setUniform("color", bar_c)
            life_sha.shad:use(true)
        end
        ...
        if player:attr("time_shield") then
            shield = shield + (player.time_shield_absorb or 0)
            max_shield = max_shield + (player.time_shield_absorb_max or 0)
        end
        if player:attr("damage_shield") then
            shield = shield + (player.damage_shield_absorb or 0)
            max_shield = max_shield + (player.damage_shield_absorb_max or 0)
        end
        if player:attr("displacement_shield") then
            shield = shield + (player.displacement_shield or 0)
            max_shield = max_shield + (player.displacement_shield_max or 0)
        end
        if player:attr("disruption_shield_power") then
            shield = shield + (player.disruption_shield_power or 0)
            max_shield = max_shield + (player:callTalent(player.T_DISRUPTION_SHIELD, "getMaxAbsorb") or 0)
        end
        local necroshield = player:isTalentActive(player.T_HIEMAL_SHIELD)
        if necroshield then
            shield = shield + (necroshield.shield or 0)
            max_shield = max_shield + (necroshield.original_shield or 0)
        end

        local front = fshat_life_dark
        if max_shield > 0 then
            front = fshat_shield_dark
            if shield >= max_shield * 0.8 then
                front = fshat_shield
            end
        elseif player.life >= player.max_life then
            front = fshat_life
        end
        front[1]:toScreenFull(x, y, front[6], front[7], front[2], front[3], 1, 1, 1, a)
        ...
        if max_shield > 0 then
            if shield_sha.shad then
                shield_sha:setUniform("a", a * 0.5)
                shield_sha.shad:use(true)
            end
            ...
            if game.mouse:getZone("res:life") then
                game.mouse:unregisterZone("res:life")
            end
        else
            self:showResourceTooltip(bx + x * scale, by + y * scale, fshat[6], fshat[7], "res:life", self.TOOLTIP_LIFE, true)
            if game.mouse:getZone("res:shield") then
                game.mouse:unregisterZone("res:shield")
            end
        end
        ...
        for res, res_def in ipairs(ActorResource.resources_def) do
            ...
            if not res_def.hidden_resource then
                if player:knowTalent(res_def.talent) and not player["_hide_resource_" .. res_def.short_name] then
                    ...
                    if rshad_args.display_resource_bar then
                        -- use a resource-specific method
                        rshad_args.display_resource_bar(player, rgfx.shader, x, y, rgfx.color, a)
                    else
                        -- use the default method, as fraction of maximum
                    ...
                    x, y = self:resourceOrientStep(orient, bx, by, scale, x, y, fshat[6], fshat[7])
                elseif game.mouse:getZone("res:" .. rname) then
                    game.mouse:unregisterZone("res:" .. rname)
                end
            end
        end
        ...
        if player.psionic_feedback_max and player:knowTalent(player.T_FEEDBACK_POOL) and not player._hide_resource_feedback then
            sshat[1]:toScreenFull(x - 6, y + 8, sshat[6], sshat[7], sshat[2], sshat[3], 1, 1, 1, a)
            bshat[1]:toScreenFull(x, y, bshat[6], bshat[7], bshat[2], bshat[3], 1, 1, 1, a)
            if feedback_sha.shad then
                feedback_sha:setUniform("a", a)
                feedback_sha.shad:use(true)
            end
            ...
            if feedback_sha.shad then
                feedback_sha.shad:use(false)
            end

            local front = fshat_feedback_dark
            if player.psionic_feedback >= player.psionic_feedback_max then
                front = fshat_feedback
            end
            front[1]:toScreenFull(x, y, front[6], front[7], front[2], front[3], 1, 1, 1, a)
            ...
            x, y = self:resourceOrientStep(orient, bx, by, scale, x, y, fshat[6], fshat[7])
        elseif game.mouse:getZone("res:feedback") then
            game.mouse:unregisterZone("res:feedback")
        end
        ...
        if player.is_fortress and not player._hide_resource_fortress then
            ...
            x, y = self:resourceOrientStep(orient, bx, by, scale, x, y, fshat[6], fshat[7])
        elseif game.mouse:getZone("res:fortress") then
            game.mouse:unregisterZone("res:fortress")
        end
        ...
        if ammo then
            local amt, max = 0, 0
            local shad, bg
            if ammo.type == "alchemist-gem" or ammo.type == "gem" then
            ...
            x, y = self:resourceOrientStep(orient, bx, by, scale, x, y, fshat[6], fshat[7])
        elseif game.mouse:getZone("res:ammo") then
            game.mouse:unregisterZone("res:ammo")
        end
        ...
        elseif game.mouse:getZone("res:hourglass") then
            game.mouse:unregisterZone("res:hourglass")
        end
        ...
        elseif game.mouse:getZone("res:levelspec") then
            game.mouse:unregisterZone("res:levelspec")
        end
        ...
        if savefile_pipe.saving then
            sshat[1]:toScreenFull(x - 6, y + 8, sshat[6], sshat[7], sshat[2], sshat[3], 1, 1, 1, a)
            bshat[1]:toScreenFull(x, y, bshat[6], bshat[7], bshat[2], bshat[3], 1, 1, 1, a)
            if save_sha.shad then
                save_sha:setUniform("a", a)
                save_sha.shad:use(true)
            end
...
function _M:buffOrientStep(orient, bx, by, scale, x, y, w, h, next)
    if orient == "down" or orient == "up" then
        x = x + w
        if (x + w) * scale >= game.w - bx or next then
            x = 0
            y = y + h * (orient == "down" and 1 or -1)
        end
    elseif orient == "right" or orient == "left" then
        y = y + h
        if (y + h) * scale >= self.map_h_stop - by or next then
            y = 0
            x = x + w * (orient == "right" and 1 or -1)
        end
    end
    return x, y
end
...
function _M:handleEffect(player, eff_id, e, p, x, y, hs, bx, by, is_first, scale, allow_remove)
    ...
    if not self.tbuff[eff_id .. ":" .. dur .. ":" .. charges] then
        ...
        if e.display_desc then
            name = e.display_desc(self, p)
        end
        ...
        local desc_fct = function(button, mx, my, xrel, yrel, bx, by, event)
            if is_first then
                if event == "out" then
                    self.mhandle.buffs = nil
                    return
                else
                    self.mhandle.buffs = true
                end

                -- Move handle
                if not self.locked and bx >= self.mhandle_pos.buffs.x and bx <= self.mhandle_pos.buffs.x + move_handle[6] and by >= self.mhandle_pos.buffs.y and by <= self.mhandle_pos.buffs.y + move_handle[7] then
                    self:uiMoveResize("buffs", button, mx, my, xrel, yrel, bx, by, event)
                end
            end
            ...
            game.tooltip_x, game.tooltip_y = 1, 1;
            game:tooltipDisplayAtMap(game.w, game.h, desc)
        end
        ...
        self.tbuff[eff_id .. ":" .. dur .. ":" .. charges] = { eff_id, "tbuff" .. eff_id, function(x, y)
            ...
            if shader and (txt or txt2) then
                shader:use(false)
            end

            if flash > 0 then
                if e.status ~= "detrimental" then
                    core.display.drawQuad(x + 4, y + 4, 32, 32, 0, 255, 0, 170 - flash * 30)
                else
                    core.display.drawQuad(x + 4, y + 4, 32, 32, 255, 0, 0, 170 - flash * 30)
                end
                flash = flash - 1
            end
        end, desc_fct }
...
function _M:displayBuffs(scale, bx, by)
    ...
    if player then
        if player.changed then
            for _, d in pairs(self.pbuff) do
                if not player.sustain_talents[d[1]] then
                    game.mouse:unregisterZone(d[2])
                end
            end
            for _, d in pairs(self.tbuff) do
                if not player.tmp[d[1]] then
                    game.mouse:unregisterZone(d[2])
                end
            end
            self.tbuff = {}
        ...
        for tid, act in pairs(player.sustain_talents) do
            if act then
                if not self.pbuff[tid] or act.__update_display then
                    ...
                    if act.__update_display then
                        game.mouse:unregisterZone("pbuff" .. tid)
                    end
                    act.__update_display = false
                    local displayName = t.name
                    if t.getDisplayName then
                        displayName = t.getDisplayName(player, t, player:isTalentActive(tid))
                    end
...
function _M:displayParty(scale, bx, by)
    if game.player.changed and next(self.party) then
        for a, d in pairs(self.party) do
            if not game.party:hasMember(a) then
                game.mouse:unregisterZone(d[2])
            end
        end
        self.party = {}
        ...
        for i = 1, #game.party.m_list do
            ...
            if not self.party[a] then
                ...
                local desc_fct = function(button, mx, my, xrel, yrel, bx, by, event)
                    if is_first then
                        if event == "out" then
                            self.mhandle.party = nil
                            return
                        else
                            self.mhandle.party = true
                        end
                        -- Move handle
                    ...
                    if event == "button" and button == "left" then
                        if def.control == "full" then
                            game.party:select(a)
                        elseif def.orders then
                            game.party:giveOrders(a)
                        end
    ...
    if not game.mouse:updateZone("pframe", bx, by, pf_bg[6], pf_bg[7], nil, scale) then
        ...
        local desc_fct = function(button, mx, my, xrel, yrel, bx, by, event)
            ...
            elseif bx >= 22 and bx <= 22 + 40 and by >= 22 and by <= 22 + 40 then
                game.tooltip_x, game.tooltip_y = 1, 1;
                game:tooltipDisplayAtMap(game.w, game.h, _t "Show character infos")
                if event == "button" and button == "left" then
            ...
            elseif bx >= 269 and bx <= 269 + pf_levelup[6] and by >= 78 and by <= 78 + pf_levelup[7] and (player.unused_stats > 0 or player.unused_talents > 0 or player.unused_generics > 0 or player.unused_talents_types > 0) then
                game.tooltip_x, game.tooltip_y = 1, 1;
                game:tooltipDisplayAtMap(game.w, game.h, _t "Click to assign stats and talents!")
                if event == "button" and button == "left" then
            ...
            elseif bx >= 298 + pf_mtx_x and bx <= 298 + pf_mtx_x + pf_mtx[6] and by >= 6 + pf_mtx_y and by <= 6 + pf_mtx_y + pf_mtx[7] and profile:canMTXN() then
                game.tooltip_x, game.tooltip_y = 1, 1;
                game:tooltipDisplayAtMap(game.w, game.h, _t "Show available cosmetic & fun microtransation")
                if event == "button" and button == "left" then
...
function _M:displayMinimap(scale, bx, by)
    ...
    if not game.mouse:updateZone("minimap", bx, by, mm_bg[6], mm_bg[7], nil, scale) then
        ...
        local desc_fct = function(button, mx, my, xrel, yrel, bx, by, event)
            if event == "out" then
                self.mhandle.minimap = nil
                return
            else
                self.mhandle.minimap = true
            end
            if self.no_minimap then
                return
            end
            ...
            game.tooltip_x, game.tooltip_y = 1, 1;
            game:tooltipDisplayAtMap(game.w, game.h, _t "Left mouse to move\nRight mouse to scroll\nMiddle mouse to show full map")
...
function _M:displayGameLog(scale, bx, by)
    ...
    if not game.mouse:updateZone("gamelog", bx, by, log.w, log.h, nil, scale) then
        ...
        local desc_fct = function(button, mx, my, xrel, yrel, bx, by, event)
            if event == "out" then
                self.mhandle.gamelog = nil
                return
            else
                self.mhandle.gamelog = true
            end
...
function _M:displayChatLog(scale, bx, by)
    ...
    if not game.mouse:updateZone("chatlog", bx, by, log.w, log.h, nil, scale) then
        ...
        local desc_fct = function(button, mx, my, xrel, yrel, bx, by, event)
            if event == "out" then
                self.mhandle.chatlog = nil
                return
            else
                self.mhandle.chatlog = true
            end
            ...
            self.hotkeys_display:onMouse(button, mx, my, event == "button",
                    ...
                    function(i, hk)
                        ...
                        elseif button == "right" and hk and hk[1] == "inventory" then
                            Dialog:yesnoPopup(("Unbind %s"):tformat(hk[2]), _t "Remove this object from your hotkeys?", function(ret)
                                if ret then
                                    for i = 1, 12 * game.player.nb_hotkey_pages do
                                        if game.player.hotkey[i] and game.player.hotkey[i][1] == "inventory" and game.player.hotkey[i][2] == hk[2] then
                                            game.player.hotkey[i] = nil
                                        end
                                    end
                                end
                            end)
...
function _M:displayToolbar(scale, bx, by)
    ...
    if not game.mouse:updateZone("tb_inven", bx + x * scale, by + y * scale, tb_inven[6], tb_inven[7], nil, scale) then
        ...
        local desc_fct = function(button, mx, my, xrel, yrel, bx, by, event)
            if event == "out" then
                self.tbbuttons.inven = 0.6
                return
            else
                self.tbbuttons.inven = 1
            end
            game.tooltip_x, game.tooltip_y = 1, 1;
            game:tooltipDisplayAtMap(game.w, game.h, _t "Left mouse to show inventory\nRight mouse to show ingredients")
            if button == "left" and not xrel and not yrel and event == "button" then
                game.key:triggerVirtual("SHOW_INVENTORY")
            elseif button == "right" and not xrel and not yrel and event == "button" then
                game:registerDialog(require("mod.dialogs.ShowIngredients").new(game.party))
            end
        end
    ...
    if not game.mouse:updateZone("tb_talents", bx + x * scale, by + y * scale, tb_talents[6], tb_talents[7], nil, scale) then
        ...
        local desc_fct = function(button, mx, my, xrel, yrel, bx, by, event)
            if event == "out" then
                self.tbbuttons.talents = 0.6
                return
            else
                self.tbbuttons.talents = 1
            end
            game.tooltip_x, game.tooltip_y = 1, 1;
            game:tooltipDisplayAtMap(game.w, game.h, _t "Left mouse to show known talents")
            if button == "left" and not xrel and not yrel and event == "button" then
    ...
    if not game.mouse:updateZone("tb_quest", bx + x * scale, by + y * scale, tb_quest[6], tb_quest[7], nil, scale) then
        ...
        local desc_fct = function(button, mx, my, xrel, yrel, bx, by, event)
            if event == "out" then
                self.tbbuttons.quest = 0.6
                return
            else
                self.tbbuttons.quest = 1
            end
            game.tooltip_x, game.tooltip_y = 1, 1;
            game:tooltipDisplayAtMap(game.w, game.h, _t "Left mouse to show message/chat log.")
            if button == "left" and not xrel and not yrel and event == "button" then
    ...
    if not game.mouse:updateZone("tb_lore", bx + x * scale, by + y * scale, tb_lore[6], tb_lore[7], nil, scale) then
        ...
        local desc_fct = function(button, mx, my, xrel, yrel, bx, by, event)
            if event == "out" then
                self.tbbuttons.lore = 0.6
                return
            else
                self.tbbuttons.lore = 1
            end
            game.tooltip_x, game.tooltip_y = 1, 1;
            game:tooltipDisplayAtMap(game.w, game.h, _t "Left mouse to show quest log.\nRight mouse to show all known lore.")
            if button == "left" and not xrel and not yrel and event == "button" then
                game.key:triggerVirtual("SHOW_QUESTS")
            elseif button == "right" and not xrel and not yrel and event == "button" then
                game:registerDialog(require("mod.dialogs.ShowLore").new(_t "Tales of Maj'Eyal Lore", game.party))
            end
        end
    ...
    if not game.mouse:updateZone("tb_mainmenu", bx + x * scale, by + y * scale, tb_mainmenu[6], tb_mainmenu[7], nil, scale) then
        ...
        local desc_fct = function(button, mx, my, xrel, yrel, bx, by, event)
            if event == "out" then
                self.tbbuttons.mainmenu = 0.6
                return
            else
                self.tbbuttons.mainmenu = 1
            end
            game.tooltip_x, game.tooltip_y = 1, 1;
            game:tooltipDisplayAtMap(game.w, game.h, _t "Left mouse to show main menu")
            if button == "left" and not xrel and not yrel and event == "button" then
    ...
    if not game.mouse:updateZone("padlock", bx + x * scale, by + y * scale, padlock[6], padlock[7], nil, scale) then
        ...
        local desc_fct = function(button, mx, my, xrel, yrel, bx, by, event)
            if event == "out" then
                self.tbbuttons.padlock = 0.6
                return
            else
                self.tbbuttons.padlock = 1
            end
            game.tooltip_x, game.tooltip_y = 1, 1;
            game:tooltipDisplayAtMap(game.w, game.h, self.locked and _t "Unlock all interface elements so they can be moved and resized." or _t "Lock all interface elements so they can not be moved nor resized.")
            if button == "left" and not xrel and not yrel and event == "button" then
                self:switchLocked()
            end
        end
...
function _M:display(nb_keyframes)
    ...
    if self.ui_moving and self.sizes[self.ui_moving] then
        ...
        if size.left then
            d.drawQuad(0, 0, 10, Map.viewport.height, 0, 200, 0, 50)
        end
        if size.right then
            d.drawQuad(Map.viewport.width - 10, 0, 10, Map.viewport.height, 0, 200, 0, 50)
        end
        if size.top then
            d.drawQuad(0, 0, Map.viewport.width, 10, 0, 200, 0, 50)
        end
        if size.bottom then
            d.drawQuad(0, Map.viewport.height - 10, Map.viewport.width, 10, 0, 200, 0, 50)
        end
        d.glTranslate(-Map.display_x, -Map.display_y, -0)
    end
...
function _M:setupMouse(mouse)
    ...
    self.logdisplay:onMouse(function(item, sub_es, button, event, x, y, xrel, yrel, bx, by)
        ...
        if ((not item or not sub_es or #sub_es == 0) and (not item or not item.url)) or (item and item.faded == 0) then
            game.mouse:delegate(button, mx, my, xrel, yrel, nil, nil, event, "playmap")
            return
        end
        ...
        if sub_es then
            for i, e in ipairs(sub_es) do
                if e.tooltip then
                    local t = e:tooltip()
                    if t then
                        table.append(tooltips, t)
                    end
                    if i < #sub_es then
                        table.append(tooltips, { tstring { true, "---" } })
                    else
                        table.append(tooltips, { tstring { true } })
                    end
                end
    ...
    profile.chat:onMouse(function(user, item, button, event, x, y, xrel, yrel, bx, by)
        local mx, my = core.mouse.get()
        if not item or not user or item.faded == 0 then
            game.mouse:delegate(button, mx, my, xrel, yrel, nil, nil, event, "playmap")
            return
        end
        ...
        if (user.donator and user.donator ~= "none") or (user.status and user.status == 'dev') then
            ...
            if user.status and user.status == 'dev' then
                text, color = _t "Developer", colors.CRIMSON
            elseif user.status and user.status == 'mod' then
                text, color = _t "Moderator / Helper", colors.GOLD
            elseif user.donator == "oneshot" then
                text, color = _t "Donator", colors.LIGHT_GREEN
            elseif user.donator == "recurring" then
                text, color = _t "Recurring Donator", colors.LIGHT_BLUE
            end
        ...
        if item.extra_data and item.extra_data.mode == "tooltip" then
            ...
            if button == "right" and event == "button" then
                extra.add_map_action = {
                    { name = _t "Show chat user", fct = function()
                        profile.chat:showUserInfo(user.login)
                    end },
                    { name = _t "Whisper", fct = function()
                        profile.chat:setCurrentTarget(false, user.login)
                        profile.chat:talkBox()
                    end },
                    { name = _t "Ignore", fct = function()
                        Dialog:yesnoPopup(_t "Ignore user", ("Really ignore all messages from: %s"):tformat(user.login), function(ret)
                            if ret then
                                profile.chat:ignoreUser(user.login)
                            end
                        end)
                    end },
                ...
                if profile.chat:isFriend(user.login) then
                    table.insert(extra.add_map_action, 3, { name = _t "Remove Friend", fct = function()
                        Dialog:yesnoPopup(_t "Remove Friend", ("Really remove %s from your friends?"):tformat(user.login), function(ret)
                            if ret then
                                profile.chat:removeFriend(user.login, user.id)
                            end
                        end)
                    end })
                else
                    table.insert(extra.add_map_action, 3, { name = _t "Add Friend", fct = function()
                        Dialog:yesnoPopup(_t "Add Friend", ("Really add %s to your friends?"):tformat(user.login), function(ret)
                            if ret then
                                profile.chat:addFriend(user.login, user.id)
                            end
                        end)
                    end })
                end

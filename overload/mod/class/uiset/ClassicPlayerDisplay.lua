...
function _M:makeTextureBar(text, nfmt, val, max, reg, x, y, r, g, b, bar_col, bar_bgcol)
    ...
    for i = 1, #items do
        if type(items[i]) == "table" then
            ...
            self.items[#self.items + 1] = function(dx, dy)
                return items[i](dx + x, dy + y)
            end
        end
    end
...
function _M:handleEffect(eff_id, e, p, ex, h)
    ...
    local remove_fct = function(button)
    ...
    if config.settings.tome.effects_icons and e.display_entity then
        ...
        if e.status == "detrimental" then
            ...
            self:mouseTooltip(desc, _w, _h, _x, _y, config.settings.cheat and remove_fct or nil)
            h = h + self.font_h
        else
            ...
            self:mouseTooltip(desc, _w, _h, _x, _y, ((e.status == "beneficial" and not e.no_player_remove) or config.settings.cheat) and remove_fct or nil)
            h = h + self.font_h
        end
    end
    return ex, h
...
function _M:display()
    local player = game.player
    if not player or not player.changed or not game.level then
        return
    end
    ...
    self.font:setStyle("bold")
    self:makeTexture(("%s#{normal}#"):tformat(player.name), 0, h, colors.GOLD.r, colors.GOLD.g, colors.GOLD.b, self.w)
    h = h + self.font_h
    self.font:setStyle("normal")

    self:mouseTooltip(self.TOOLTIP_LEVEL, self:makeTexture(("Level / Exp: #00ff00#%s / %2d%%"):tformat(player.level, 100 * cur_exp / max_exp), x, h, 255, 255, 255))
    h = h + self.font_h
    self:mouseTooltip(self.TOOLTIP_GOLD, self:makeTexture(("Gold: #00ff00#%0.2f"):tformat(player.money or 0), x, h, 255, 255, 255))
    h = h + self.font_h
    ...
    for i = 1, 4 do
        ...
        self:mouseTooltip(self[attack_stats[i + 4][2]], self:makeTexture(("%s"):format(text), x + 180, h, 255, 255, 255))
        h = h + self.font_h
    end
    h = h + self.font_h

    if game.level and game.level.turn_counter then
        self:makeTexture(("Turns remaining: %d"):tformat(game.level.turn_counter / 10), x, h, 255, 0, 0)
        h = h + self.font_h
        h = h + self.font_h
    end

    if player:getAir() < player.max_air then
        self:mouseTooltip(self.TOOLTIP_AIR, self:makeTexture(("Air level: %d/%d"):tformat(player:getAir(), player:getMaxAir()), x, h, 255, 0, 0))
        h = h + self.font_h
        h = h + self.font_h
    end

    if player:attr("encumbered") then
        self:mouseTooltip(self.TOOLTIP_ENCUMBERED, self:makeTexture(("Encumbered! (%d/%d)"):tformat(player:getEncumbrance(), player:getMaxEncumbrance()), x, h, 255, 0, 0))
        h = h + self.font_h
        h = h + self.font_h
    end

    self:mouseTooltip(self.TOOLTIP_STRDEXCON, self:makeTexture(("Str/Dex/Con: #00ff00#%3d/%3d/%3d"):tformat(player:getStr(), player:getDex(), player:getCon()), x, h, 255, 255, 255))
    h = h + self.font_h
    self:mouseTooltip(self.TOOLTIP_MAGWILCUN, self:makeTexture(("Mag/Wil/Cun: #00ff00#%3d/%3d/%3d"):tformat(player:getMag(), player:getWil(), player:getCun()), x, h, 255, 255, 255))
    h = h + self.font_h
    h = h + self.font_h

    self:mouseTooltip(self.TOOLTIP_LIFE, self:makeTextureBar(_t "#c00000#Life    :", nil, player.life, player.max_life, player.life_regen * util.bound((player.healing_factor or 1), 0, 2.5), x, h, 255, 255, 255, colors.DARK_RED, colors.VERY_DARK_RED))
    h = h + self.font_h

    local shield, max_shield = 0, 0
    if player:attr("time_shield") then
        shield = shield + player.time_shield_absorb
        max_shield = max_shield + player.time_shield_absorb_max
    end
    if player:attr("damage_shield") then
        shield = shield + player.damage_shield_absorb
        max_shield = max_shield + player.damage_shield_absorb_max
    end
    if player:attr("displacement_shield") then
        shield = shield + player.displacement_shield
        max_shield = max_shield + player.displacement_shield_max
    end
    if max_shield > 0 then
        self:mouseTooltip(self.TOOLTIP_DAMAGE_SHIELD, self:makeTextureBar(_t "#WHITE#Shield:", nil, shield, max_shield, nil, x, h, 255, 255, 255, { r = colors.GREY.r / 3, g = colors.GREY.g / 3, b = colors.GREY.b / 3 }, { r = colors.GREY.r / 6, g = colors.GREY.g / 6, b = colors.GREY.b / 6 }))
        h = h + self.font_h
    end
    ...
    for res, res_def in ipairs(ActorResource.resources_def) do
        if not res_def.hidden_resource and player:knowTalent(res_def.talent) then
            -- only show resources if the player knows the appropriate pool
...
%s
    ...
    if (player.unnatural_body_heal or 0) > 0 and player:knowTalent(player.T_UNNATURAL_BODY) then
        ...
        self:mouseTooltip(self.TOOLTIP_UNNATURAL_BODY, self:makeTextureBar(_t "#c00000#Un.body :", ("%0.1f (%0.1f/turn)"):tformat(player.unnatural_body_heal, math.min(regen, player.unnatural_body_heal)), regen, player.unnatural_body_heal, nil, x, h, 255, 255, 255, colors.DARK_RED, colors.VERY_DARK_RED))
        h = h + self.font_h
    end
    ...
    if ammo then
        if ammo.type == "alchemist-gem" then
            self:mouseTooltip(self.TOOLTIP_COMBAT_AMMO, self:makeTexture(("#ANTIQUE_WHITE#Ammo    :       #ffffff#%d"):tformat(ammo:getNumber()), 0, h, 255, 255, 255))
            h = h + self.font_h
        elseif ammo.type == "gem" then
            self:mouseTooltip(self.TOOLTIP_COMBAT_AMMO, self:makeTexture(("#ANTIQUE_WHITE#Ammo    :       #ffffff#%s"):tformat(ammo:getName()), 0, h, 255, 255, 255))
            h = h + self.font_h
        else
            self:mouseTooltip(self.TOOLTIP_COMBAT_AMMO, self:makeTexture(("#ANTIQUE_WHITE#Ammo    :       #ffffff#%d/%d"):tformat(ammo.combat.shots_left, ammo.combat.capacity), 0, h, 255, 255, 255))
            h = h + self.font_h
        end
    end
    ...
    for tid, act in pairs(player.sustain_talents) do
        if act then
            ...
            if config.settings.tome.effects_icons and t.display_entity then
                ...
                self:mouseTooltip(desc, self:makeTexture(("#LIGHT_GREEN#%s"):format(displayName), x, h, 255, 255, 255))
                h = h + self.font_h
                ex = 0
            end
        end
    end
    if config.settings.tome.effects_icons then
        h = h + 40
        ex = 0
    end
    local good_e, bad_e = {}, {}
    ...
    if game.level and game.level.arena then
        ...
        if arena.score > world.arena.scores[1].score then
            self:makeTexture(("Score(TOP): %d"):tformat(arena.score), x, h, 255, 255, 100)
            h = h + self.font_h
        else
            self:makeTexture(("Score: %d"):tformat(arena.score), x, h, 255, 255, 255)
            h = h + self.font_h
        end
        ...
        if arena.pinch == true then
            self:makeTexture(("Bonus: %d (x%.1f)"):tformat(arena.bonus, arena.bonusMultiplier), x, h, 255, 50, 50)
            h = h + self.font_h
        else
            self:makeTexture(("Bonus: %d (x%.1f)"):tformat(arena.bonus, arena.bonusMultiplier), x, h, 255, 255, 255)
            h = h + self.font_h
        end
        if arena.display then
            h = h + self.font_h
            self:makeTexture(arena.display[1], x, h, 255, 0, 255)
            h = h + self.font_h
            self:makeTexture(_t " VS", x, h, 255, 0, 255)
            h = h + self.font_h
            self:makeTexture(arena.display[2], x, h, 255, 0, 255)
            h = h + self.font_h
        else
            self:makeTexture(("Rank: %s"):tformat(arena.printRank(arena.rank, arena.ranks)), x, h, 255, 255, 255)
            h = h + self.font_h
        end
        h = h + self.font_h
    end

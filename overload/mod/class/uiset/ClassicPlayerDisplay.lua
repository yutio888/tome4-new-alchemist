...
%s
    ...
    if ammo then
        ...
        elseif ammo.type == "gem" then
            self:mouseTooltip(self.TOOLTIP_COMBAT_AMMO, self:makeTexture(("#ANTIQUE_WHITE#Ammo    :       #ffffff#%s"):tformat(ammo:getName()), 0, h, 255, 255, 255)) h = h + self.font_h
        else
    ...
    for eff_id, p in pairs(player.tmp) do
        ...
        if e.status == "detrimental" then bad_e[eff_id] = p
        else good_e[eff_id] = p end
    end
